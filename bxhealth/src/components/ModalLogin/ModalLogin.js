...
class ModalLogin extends React.Component {
  ...
  render() {
    if (this.state.loggedIn) {
      return <Redirect push to="/dashboard?caregiver=true" />;
    }
    const closeBtn = <div />;
    return (
      <div>
          ...
          <ModalBody>
            <form>
              <TabContent activeTab={this.state.activeTab}>
                <TabPane tabId="1">
                  { this.props.caregiver == "true" &&
                    <h4>{data.titles.welcome_caregiver}</h4>                  
                  }
                  { this.props.caregiver == "false" &&
                  ...
                  <div className="mb-3">
                    <Button type="button" color="primary" onClick={() => { this.props.caregiver == "true" ? this.setlLoggedIn() : this.toggleTab('2'); }}>{data.form.buttons.next}</Button>
                  </div>
                  <div>
                ...
                <TabPane tabId="3">
                  <div className="mobile-loading" style={{backgroundImage: `url(${process.env.PUBLIC_URL}/images/login-device-outline.jpg)`}}>
                    ...
                    <p dangerouslySetInnerHTML={{__html: data.mobile.loading}}></p>
                  </div>
